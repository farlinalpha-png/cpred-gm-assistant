# Edgerunner Folio  Fillable

<!-- Page 1 -->

edgerunner folio space for all your character’s details
BY SEBASTIAN SZMYD
Design by J Gray•Art Direction by Winterjaye Kovach
Business Management by Lisa Pondsmith•Layout by J Gray
Copyright © 2024 by R. Talsorian Games, Inc., Cyberpunk is a registered trademark of CD Projekt Red S.A. All rights reserved under the Universal Copyrights Convention. 
All situations, governments, and people herein are fictional. Any similarities portrayed herein without satiric intent are strictly coincidental.
Permission granted to the user to print and copy this document for personal use.
JULY 2024 V1.0
1

---

<!-- Page 2 -->

INT
REF
DEX
TECH
COOL
WILL
MOVE
BODY
EMP
Handle
Role
HP S Wounded
Death Save
out of
Luck
-2 to all Checks whenHP ≤Above
ARMOR
Body
If damage penetrates armor, theSP goes down by 1
vsRanged/
Brawling
vsMelee/
MartialArts
Head
ATTACKS
SKILLS
A
Skill Check = 1d10 + BASE + 
AnyBonuses Not
Already Included in theBase
Weapon Skill Base (STAT + SKILL + BONUS) DMG Ammo ROF Notes
Name STAT LVL BASE Name STAT LVL BASE
vsRanged/
Brawling
vsMelee/
MartialArts
Role Ability Rank
HUMANITY Reputation
out of
out of

---

<!-- Page 3 -->

CYBERWARE
Name Category
Name Category
Name Category
Name Category
Name Category
Notes
Notes
Notes
Notes
Notes
Additional Notes

---

<!-- Page 4 -->

CYBERWARE
Name Category
Name Category
Name Category
Name Category
Name Category
Notes
Notes
Notes
Notes
Notes
Additional Notes

---

<!-- Page 5 -->

ATTACKS
Brawling
Hand-to-hand (or foot or headbutt or elbow... you get the idea) combat. You can do the following Actions with it.
• Attack: A straight up punch/kick/bodyblow. Does damage based on BODY. ROF2.
• Grab: Initiate a grapple. Your Brawling Base vs your opponent’s Brawling Base. If you win, you now have 
them in a grapple. Anyone involved in the grapple takes a -2 to all Checks. You can drop the grapple at any 
time without an Action. ROF1.
• Choke: If you have a target in a grapple, you can automatically do damage to them equal to your BODY 
without a Check. This is done directly to their HP and not affected by armor. If you choke them for 3 
Rounds in a row, they pass out regardless of how much HP they have left. ROF1.
• Slam: If you have a target in a grapple, you can automatically slam them into the ground without a Check. 
When you slam an opponent, you leave them in an adjacent square of your choice and damage to them 
equal to your BODY. This is done directly to their HP and not affected by armor. ROF1.
Additional Notes
DV based on range from target (in m/yds)
0-6 7-12 13-25 26-50 51-100 101-200 201-400 401-800
Name
Type Skill Base (STAT + SKILL + BONUS) DMG Ammo ROF Hands Concealable?
Notes
DV based on range from target (in m/yds)
0-6 7-12 13-25 26-50 51-100 101-200 201-400 401-800
Name
Type Skill Base (STAT + SKILL + BONUS) DMG Ammo ROF Hands Concealable?
Notes

---

<!-- Page 6 -->

Additional Notes
DV based on range from target (in m/yds)
0-6 7-12 13-25 26-50 51-100 101-200 201-400 401-800
Name
Type Skill Base (STAT + SKILL + BONUS) DMG Ammo ROF Hands Concealable?
Notes
DV based on range from target (in m/yds)
0-6 7-12 13-25 26-50 51-100 101-200 201-400 401-800
Name
Type Skill Base (STAT + SKILL + BONUS) DMG Ammo ROF Hands Concealable?
Notes
DV based on range from target (in m/yds)
0-6 7-12 13-25 26-50 51-100 101-200 201-400 401-800
Name
Type Skill Base (STAT + SKILL + BONUS) DMG Ammo ROF Hands Concealable?
Notes
ATTACKS

---

<!-- Page 7 -->

GEAR
Name Notes
Cash
AMMUNITION
Name Weapon Type Ammo Type Amount Notes
HOUSING & LIFESTYLE
Type Notes
Improvement Points

---

<!-- Page 8 -->

Name
VEHICLE SHEET
STATS
Seats
NUMBER INSTALLED
NUMBER INSTALLED
out ofout of
Type
Control Skill Repair Skill
SDP SP Speed (Combat) Speed (Narrative)
DV based on range from target (in m/yds)
0-6 7-12 13-25 26-50 51-100 101-200 201-400 401-800
DV based on range from target (in m/yds)
0-6 7-12 13-25 26-50 51-100 101-200 201-400 401-800
Name
Name
Type Skill Base (STAT + SKILL + BONUS) DMG Ammo ROF Hands Concealable?
Type Skill Base (STAT + SKILL + BONUS) DMG Ammo ROF Hands Concealable?
Attacks
Installed Upgrades

---

<!-- Page 9 -->

Name
CYBERDECK SHEET
STATS
out ofout ofout of
Type
Universal Slots Program Slots Hardware Slots
Raven 
Sabertooth 
Scorpion 
Skunk 
Wisp 
NUMBER LOADED NUMBER LOADED
NUMBER LOADED NUMBER LOADED
Programs Loaded
Black ICE Loaded
LOCATION
LOCATION
Hardware Installed

---

<!-- Page 10 -->

Hair Style Affectation
What Do You Value Most? Feelings About People?
Most Valued Person Most Valued Possession
Family Background Childhood Environment
Family Crisis
Friends Tragic Love Affairs
Enemies
Life Goals
Who? What Caused It? What Can They Throw at You? What’s Gonna Happen?
LIFEPATH
Personality Clothing Style
Cultural Origins
Additional Notes
Role Specific Lifepath

---

<!-- Page 11 -->

Attacker’s STAT + Skill + 1d10 vs. Defender’s STAT + Skill + 1d10 or Task Difficulty Value (DV)
You must beat the DV (surpass the DV by 1 or higher) and not just meet the DV.
Task DVs: 9 (Simple) • 13 (Everyday) • 15 (Difficult) • 17 (Professional)
21 (Heroic) • 24 (Incredible) • 29 (Legendary)
SKILL CHECKS
▶ Check Explosion ◀
If you roll a 10 on a 1d10 for a Check, roll again and add the second roll to the first. Do not roll again if you roll another 10.
▶ Check Implosion ◀
If you roll a 1 on a 1d10 for a Check, roll again and subtract the second roll from 
your STAT + Skill + 1st roll . Do not roll again if you roll another 1.
▶ Trying Again ◀
If you fail a Check, you can’t try again unless your chances of success have improved for some reason.
▶ Complimentary Skills ◀
With GM approval, a Character can make a single complimentary Skill Check to give a +1 bonus to the main Skill Check.
▶ Taking Extra Time ◀
Get a single +1 bonus to a Skill Check by spending 4x the required time on it.
▶ Using LUCK ◀
Each point of LUCK spent gives +1 to a Skill Check. 
When a combat starts, 
everyone rolls Initiative.
REF + 1d10
▶ On Your Turn ◀
Your Turn = 1 Move Action + 1 other Action
▶ Move Actions ◀
On your Turn you can move a number 
of m/yds = to your MOVE x2 or a 
number of squares = your MOVE.
You can split your Move Action to 
perform Check between movements 
(move, shoot, move).
Special movement (climbing, jumping, 
swimming, etc) is done at 1/2 MOVE.
INITIATIVE & MOVEMENT
Attacker’s REF + Relevant Weapon Skill + 1d10
vs. DV Determined by Range & Weapon
Attacker’s DEX + Relevant Melee Attack Skill + 1d10 
vs. Defender’s DEX + Evasion Skill + 1d10
RANGED ATTACKS
MELEE ATTACKS
As part of an Attack Action, you can make 
a number of Checks = to the ROF. 
This can be split across two weapons as 
long as both weapons are ROF 2.
RATE OF FIRE (ROF)
 -8 to the Attack Check in exchange for specifically aiming for the head, held item, or leg. ROF 1.
Can aim Melee and Ranged Attacks.
AIMED SHOTS
